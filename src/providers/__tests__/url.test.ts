...
describe('resolveApiBase', () => {
  ...
  it('keeps /v1 in base URL', () => {
    expect(resolveApiBase('https://gpu1-llm.emofid.com/v1/')).toBe('https://gpu1-llm.emofid.com/v1');
  });
});

describe('buildEndpoint', () => {
  it('defaults empty path to chat completions under /v1 base', () => {
    expect(buildEndpoint('https://gpu1-llm.emofid.com/v1/', '')).toBe(
      'https://gpu1-llm.emofid.com/v1/chat/completions',
    );
  });

  it('lists models without duplicating /v1', () => {
    expect(buildEndpoint('https://gpu1-llm.emofid.com/v1/', '/models')).toBe(
      'https://gpu1-llm.emofid.com/v1/models',
    );
  });
